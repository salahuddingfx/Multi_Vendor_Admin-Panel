    ...
    <div style={{ display: 'none' }}>
      <div ref={ref} className="print-standard" style={{...baseStyle, fontSize: '11px', padding: '0.75in', display: 'flex', flexDirection: 'column'}}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '40px' }}>
          <thead>
            <tr>
              <th style={{ backgroundColor: '#334155', color: '#fff', padding: '8px 10px', textAlign: 'center', fontSize: '11px', fontWeight: 600, width: '50px' }}>SL.</th>
              <th style={{ backgroundColor: '#334155', color: '#fff', padding: '8px 10px', textAlign: 'left', fontSize: '11px', fontWeight: 600 }}>Item Description</th>
              <th style={{ backgroundColor: '#334155', color: '#fff', padding: '8px 10px', textAlign: 'center', fontSize: '11px', fontWeight: 600 }}>Price</th>
              <th style={{ backgroundColor: '#334155', color: '#fff', padding: '8px 10px', textAlign: 'center', fontSize: '11px', fontWeight: 600 }}>Qty.</th>
              <th style={{ backgroundColor: '#334155', color: '#fff', padding: '8px 10px', textAlign: 'right', fontSize: '11px', fontWeight: 600 }}>Total</th>
            </tr>
          </thead>
          <tbody>
            {order.items.map((item, idx) => (
                ...
                <td style={{ padding: '8px 15px', borderLeft: '1px solid #e2e8f0', borderRight: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0', color: '#1e293b' }}>
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                    {item.product?.images?.find(img => img.is_primary)?.image_path && (
                      <img 
                        src={item.product.images.find(img => img.is_primary).image_path} 
                        alt="" 
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: '11px', marginBottom: '2px' }}>
                        {item.name} {item.variation_info && <span style={{ color: brandColor, fontSize: '9px' }}>({item.variation_info})</span>}
                      </div>
                      {item.product?.description && (
                        <div style={{ fontSize: '9px', color: '#64748b', fontWeight: 500, lineHeight: '1.3', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                          {item.product.description}
                        </div>
                      )}
                    </div>
